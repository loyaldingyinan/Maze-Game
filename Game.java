...
public class Game implements GameInterface {
    ...
    public void exitGame() throws RemoteException{
        ...
        if(this.isPrimary){
            ...
            GameInterface secondaryStub = this.playerList.getPlayer(this.secondary).getStub();
            secondaryStub.setIsPrimary(true);
            secondaryStub.setNewSecondary(this.getRandomPlayer());
            exitCheck = secondaryStub.exitGameServer(this.player,ip,portNo);
            
        }
        else if(this.isSecondary){
            
            GameInterface primaryStub = this.playerList.getPlayer(this.primary).getStub();
            primaryStub.setNewSecondary(getRandomPlayer());
            exitCheck = primaryStub.exitGameServer(this.player,ip,portNo);
        ...
        if(exitCheck){
            System.err.println("Exit game successfully!");
        }
        
        else{
            System.err.println("Couldn't exit game.");
        }
    }
    
    public boolean exitGameServer(Player player, String ip, int portNo){
        if(this.isPrimary()){
            try{
                Registry registry = LocateRegistry.getRegistry(ip, portNo);
                TrackerInterface trackerStub = (TrackerInterface) registry.lookup("Tracker");
                
                trackerStub.removePlayer(player.getplayerID());
                
                this.playerList.removePlayer(player.getplayerID());
                
                this.gameState.removePlayer(player.getplayerID());
                
                //this.state.removePlayer(player);
                
                GameInterface secondaryStub = this.playerList.getPlayer(this.secondary).getStub();
                secondaryStub.setPlayerList(this.playerList);
                secondaryStub.setGameState(this.gameState);
                //secondaryStub.setState(this.state);
                
                return true;
                
            } catch (Exception e) {
                System.err.println("Couldn't exit the game: " + e.toString());
                e.printStackTrace();
                return false;
            }
        }
            ...
            System.err.println("Can only exit game via the primary server");
            return false;
        }
        
    }
    
    public void setNewSecondary(String secondID) throws RemoteException{
        if(isPrimary()){
            this.setSecondary(secondID);
            GameInterface secondaryStub = this.playerList.getPlayer(secondID).getStub();
            secondaryStub.setIsSecondary(true);
        }
    }
    /**
     * Helper method.
